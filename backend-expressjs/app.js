...
app.post('/chat', async (req, res) => {
  ...
  // Proxy /search to FastAPI /search
  app.post('/search', async (req, res) => {
    const { query_text } = req.body;
    if (!query_text) return res.status(400).json({ error: 'query_text is required' });

    try {
      const response = await axios.post(`${FASTAPI_URL}/search`, { query_text });
      res.json(response.data);
    } catch (err) {
      console.error('Error proxying to FastAPI /search:', err.message || err);
...
// Proxy /adk_query to FastAPI /adk_query
app.post('/adk_query', async (req, res) => {
  const { query_text } = req.body;
  if (!query_text) return res.status(400).json({ error: 'query_text is required' });

  try {
    const response = await axios.post(`${FASTAPI_URL}/adk_query`, { query_text });
    res.json(response.data);
  } catch (err) {
    console.error('Error proxying to FastAPI /adk_query:', err.message || err);
    res.status(500).json({ error: 'Error processing adk_query' });
  }
});
